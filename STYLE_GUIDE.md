...
8. [GraphQL Conventions](#graphql-conventions)
9. [React Conventions](#react-conventions)
10. [Interface vs Type](#interface-vs-type)
11. [Generics](#generics)
12. [Discriminated Unions](#discriminated-unions)
13. [Error Classes and Assertion Infrastructure](#error-classes-and-assertion-infrastructure)
14. [Test Files](#test-files)
...
All configurable modules use the `forRoot(options)` / `forRootAsync(options)` dynamic module pattern. Both methods must be implemented for every configurable module. `forRoot` accepts a plain options object; `forRootAsync` accepts an async options object with `useFactory`, `inject`, and optional `imports`.

```typescript
// CORRECT — both forRoot and forRootAsync must be present
@Module({})
...
All configurable modules are decorated with `@Global()` so they do not need to be explicitly imported in every child module.

### Guard registration order

Guards are evaluated in registration order. Register guards in this sequence to ensure correct behavior:

1. `GraphQLPublicGuard` — must run first; marks public endpoints and skips subsequent guards
2. `GraphQLAuthGuard` — verifies JWT; all non-public endpoints require this
3. `GraphQLRolesGuard` — checks roles; runs after auth so the user identity is available
4. `QueryComplexityGuard` — rejects expensive queries; runs after auth to allow per-user limits
5. `RateLimitGuard` — rate limiting; runs last

Registering `GraphQLRolesGuard` before `GraphQLAuthGuard` will cause role checks to fail because the user object is not yet populated.

### GraphQL operation naming

Use PascalCase for operation names in code-first resolvers. The `@Query()`, `@Mutation()`, and `@Subscription()` decorator `name` option sets the schema name; the method name is the implementation identifier.
...
// WRONG — method name used as schema name produces camelCase in schema
@Query(() => User)
...
All custom errors must extend the base error classes from `@pawells/nestjs-shared`. Use `GraphQLErrorCode` enum values (not raw strings) for error codes. Always include the original error as `cause` when rethrowing:
...
// WRONG — raw string code, no cause chain
throw new Error('not found');
```

Use `GraphQLErrorFormatter` to format errors before returning them in the Apollo error formatter callback. Do not pass raw NestJS exceptions to Apollo.

### DataLoader usage

DataLoaders must be accessed through `DataLoaderRegistry` — never construct a DataLoader directly in a resolver. The registry is request-scoped; it creates a new loader instance per request to prevent data leaking between requests.

```typescript
// CORRECT — access through registry
public async user(@Parent() post: Post, @Context() ctx: IGraphQLContext): Promise<User> {
	return ctx.loaders.users.load(post.userId);
}

// WRONG — DataLoader constructed directly in resolver
public async user(@Parent() post: Post): Promise<User> {
	const loader = new DataLoader(async (ids) => this.userService.findManyByIds(ids));
...
WebSocket subscriptions require `JwtService` to be available. Without it, all WebSocket authentication fails closed — no connection is accepted. This is intentional security behavior and must not be changed.

Use `@SubscriptionConfig` to declare subscription metadata. Filter functions must be pure (no side effects); they are called for every event and must be fast.

### Cache decorator usage

The `@Cacheable` decorator caches the return value of a resolver method. Cache keys are derived from the method arguments by default; provide a custom key function for non-trivial key derivation.

`@CacheInvalidate` invalidates cache entries by pattern. Use glob patterns carefully — overly broad patterns (e.g., `*`) will evict unrelated cache entries.

```typescript
// CORRECT — specific key pattern
@CacheInvalidate({ keyPattern: `users:${userId}:*` })

// WRONG — too broad, evicts everything
@CacheInvalidate({ keyPattern: '*' })
```

### Import ordering

Within this package, follow this import order (ESLint does not enforce order — maintain it manually):

1. Node.js built-ins (`node:crypto`, `node:util`)
2. NestJS core (`@nestjs/common`, `@nestjs/graphql`)
3. Apollo and GraphQL (`@apollo/server`, `graphql`)
4. Other external packages (`rxjs`, `joi`, `ioredis`)
5. Internal `@pawells` packages (`@pawells/nestjs-shared`, `@pawells/nestjs-auth`)
6. Relative imports within this package (`./guards/index.js`, `../types.js`)

---

## 8. GraphQL Conventions
...
## 13. Error Classes and Assertion Infrastructure
