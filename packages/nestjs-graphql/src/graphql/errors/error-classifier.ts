...
export class ErrorClassifier {
	 ...
	 */
	public static Classify(error: any): IErrorClassification {
	...
	private static IsValidationError(error: any): boolean {
		...
		if (error?.code === 'BAD_REQUEST' || error?.graphqlCode === 'BAD_USER_INPUT') {
			return true;
		}
	...
	private static IsAuthenticationError(error: any): boolean {
		...
		if (error?.name === 'UnauthorizedException' || error?.name === 'UnauthorizedError' ||
		...
		if (error?.code === 'UNAUTHENTICATED' || error?.graphqlCode === 'UNAUTHENTICATED') {
			return true;
		}
	...
	private static IsAuthorizationError(error: any): boolean {
		// Check for standard error names
		if (error?.name === 'ForbiddenException' || error?.name === 'ForbiddenError' ||
		...
		if (error?.status === this.HTTP_STATUS.FORBIDDEN || error?.statusCode === this.HTTP_STATUS.FORBIDDEN) {
			// Only classify as authorization error if we also have explicit error indicators
			if (error?.name === 'ForbiddenException' || error?.name === 'ForbiddenError' ||
				error?.code === 'FORBIDDEN' || error?.graphqlCode === 'FORBIDDEN') {
				return true;
			}
		}

		// Check for error codes
		if (error?.code === 'FORBIDDEN' || error?.graphqlCode === 'FORBIDDEN') {
			return true;
		}
		...
		if (error?.message && typeof error.message === 'string' && !error?.statusCode) {
			const MessageLower = error.message.toLowerCase().trim();
	...
	private static IsNotFoundError(error: any): boolean {
		...
		// Check for HTTP status codes
		if (error?.status === this.HTTP_STATUS.NOT_FOUND) {
			return true;
		}

		// Check for error codes
		if (error?.code === 'NOT_FOUND' || error?.graphqlCode === 'NOT_FOUND') {
			return true;
		}
	...
	private static IsConflictError(error: any): boolean {
		// Check for standard error names
		if (error?.name === 'ConflictError' || error?.constructor?.name === 'ConflictError') {
			return true;
		}

		// Check for MongoDB duplicate key error code
		if (error?.code === this.MONGODB_ERROR_CODES.DUPLICATE_KEY) {
			return true;
		}

		// Check for error codes
		if (error?.code === 'CONFLICT' || error?.graphqlCode === 'CONFLICT') {
			return true;
		}
	...
	private static IsRateLimitError(error: any): boolean {
		...
		if (error?.status === this.HTTP_STATUS.RATE_LIMIT) {
			return true;
		}

		// Check for error codes
		if (error?.code === 'RATE_LIMIT_EXCEEDED' || error?.graphqlCode === 'RATE_LIMIT_EXCEEDED') {
			return true;
		}
